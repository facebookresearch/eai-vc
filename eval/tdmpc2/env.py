        ...
        if self._enabled:
            if self._domain == "rlb":
                ...
                frame = self.env.render(
                    ...
                    height=84,
                    width=84,
                    camera_id=2 if self._domain == "quadruped" else 0,
                )
    ...
    def _reshape_obs(self, obs):
        if (
            ...
            return np.concatenate(
                [obs, np.zeros((self.observation_space.shape[0] - obs.shape[0],))]
            )
        return obs
...
def make_multitask_env(cfg):
    ...
    cfg.obs_shapes = [
        tuple(int(x) for x in obs_space.shape) for obs_space in env.observation_spaces
    ]
    cfg.action_shapes = [
        tuple(int(x) for x in action_space.shape) for action_space in env.action_spaces
    ]
