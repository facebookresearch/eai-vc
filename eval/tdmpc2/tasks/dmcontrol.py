...
class FeaturesWrapper(dm_env.Environment):
    def __init__(self, env, cfg):
        ...
        if cfg.modality == "map":
            shape = np.array(
                [
                    cfg.get("frame_stack", 1) * cfg.feature_dims[0],
                    *cfg.feature_dims[1:],
                ]
            )
        else:
            ...
            shape = np.array(
                [
                    cfg.get("frame_stack", 1) * features_to_dim[cfg.features],
                ]
            )
        self._obs_spec = specs.BoundedArray(
            shape=shape,
    ...
    def _encode(self, time_step):
        ...
        if self._cfg.modality == "map":
            _obs = _obs.view(
                self._cfg.get("frame_stack", 1) * self._cfg.feature_dims[0],
                *self._cfg.feature_dims[1:],
            )
        else:
            _obs = _obs.view(-1)
        return ExtendedTimeStep(
            observation=_obs.cpu().numpy(),
...
class TimeStepToGymWrapper(object):
    def __init__(self, env, domain, task, cfg):
        ...
        self.observation_space = gym.spaces.Box(
            low=np.full(
                obs_shp,
                -np.inf if cfg.modality != "pixels" else env.observation_spec().minimum,
            ),
            high=np.full(
                obs_shp,
                np.inf if cfg.modality != "pixels" else env.observation_spec().maximum,
            ),
            shape=obs_shp,
...
class MultitaskEnv(object):
    def __init__(self, cfg):
        ...
        if not cfg.multitask:
            ...
            assert cfg.num_tasks <= len(
                tasks
            ), f"num_tasks={cfg.num_tasks} but only {len(tasks)} tasks available"
        ...
        self._envs = [
            suite.load(
                dict(cup="ball_in_cup").get(domain, domain),
                task,
    ...
    def task_id(self, value):
        assert value is None or 0 <= value < len(
            self._envs
        ), "task_id must be in [0, {}) or None".format(len(self._envs))
    ...
    if cfg.get("distractors", False):
        ...
        env = dc_suite.load(
            ...
            background_dataset_paths=[
                "/private/home/nihansen/data/DAVIS/JPEGImages/480p"
            ],
