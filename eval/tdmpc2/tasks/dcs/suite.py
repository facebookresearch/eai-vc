...
from tasks.dcs import background
from tasks.dcs import camera
from tasks.dcs import color
from tasks.dcs import suite_utils
...
def load(
    ...
    env = suite.load(
        domain_name, task_name, task_kwargs={"random": 0}, visualize_reward=False
    )
    ...
    if difficulty:
        ...
        final_camera_kwargs.update(
            suite_utils.get_camera_kwargs(domain_name, difficulty / 2, dynamic)
        )
        if camera_kwargs:
            # Overwrite kwargs with those passed here.
    ...
    if difficulty:
        ...
        final_color_kwargs.update(
            suite_utils.get_color_kwargs(difficulty * 1.5, dynamic)
        )
        if color_kwargs:
            # Overwrite kwargs with those passed here.
