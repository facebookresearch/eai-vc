...
def clip_cam_position(
    position, min_radius, max_radius, min_h_angle, max_h_angle, min_v_angle, max_v_angle
):
...
def get_lookat_point(physics, camera_id):
    ...
    """
    dist_to_agent = (
        physics.named.data.cam_xpos[camera_id] - physics.named.data.subtree_com[1]
    )
...
class DistractingCameraEnv(control.Environment):
    ...
    **NOTE**: This wrapper should be applied BEFORE the pixel wrapper to make sure
    ...
    def setup_camera(self):
        ...
        self._roll_vel = self._random_state.uniform(
            -self._max_roll_vel, self._max_roll_vel
        )
    ...
    def _apply(self):
        ...
        self._roll_vel = np.clip(
            self._roll_vel, -self._max_roll_vel, self._max_roll_vel
        )
