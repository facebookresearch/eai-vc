...
def summary_stats(rewards):
    ...
    for k, v in stats.items():
        stats[k] = (
            round(float(v), 2) if isinstance(v, (torch.FloatTensor, float)) else v
        )
    return stats
...
class OfflineDataset(Dataset):
    def __init__(self, cfg, tasks="*", buffer=None):
        ...
        idxs = np.random.choice(
            len(self._fps), int(self._cfg.fraction * len(self._fps)), replace=False
        )
    ...
    def _partition_episodes(self, datas, cumrews):
        ...
        assert len(datas) in {
            int(1650 * self._cfg.fraction),
            int(3300 * self._cfg.fraction),
        }, "Unexpected number of episodes: {}".format(len(datas))
        train_episodes = int(
            (3280 if self._cfg.task.startswith("mw-") else 1500) * self._cfg.fraction
        )
        ...
        print(
            f"Training returns: [{cumrews[train_idxs].min():.2f}, {cumrews[train_idxs].max():.2f}]"
        )
        assert len(datas) - train_episodes > 0, "Not enough validation episodes"
        val_idxs = torch.topk(
            cumrews, k=len(datas) - train_episodes, dim=0, largest=True
        ).indices
        if self._cfg.get("use_val", False):
            print("Validation on top {} episodes".format(len(datas) - train_episodes))
            print(
                f"Validation returns: [{cumrews[val_idxs].min():.2f}, {cumrews[val_idxs].max():.2f}]"
            )
        return (
            [datas[i] for i in train_idxs],
            cumrews[train_idxs],
            train_idxs,
            [datas[i] for i in val_idxs],
            cumrews[val_idxs],
            val_idxs,
        )
...
class DMControlDataset(OfflineDataset):
    ...
    def _filter_episodes(self):
        ...
        if self._tasks != "*":
            self._fps = [
                fp for fp in self._fps if np.any([f"/{t}/" in fp for t in self._tasks])
            ]
    ...
    def _dump_histogram(self, cumulative_rewards):
        if not self._cfg.get("dump_histogram", False):
            return
        hist_fp = (
            make_dir(Path(self._cfg.logging_dir) / "histograms")
            / f"{self._cfg.task}.png"
        )
        plt.figure(figsize=(8, 4))
        plt.hist(
            cumulative_rewards, bins=400 if self._cfg.task.startswith("mw-") else 200
        )
        plt.xlabel("Episode return")
        plt.ylabel("Count")
        plt.xlim(0, 5000 if self._cfg.task.startswith("mw-") else 1000)
        plt.title(
            self._cfg.task_title.split(" ", 1)[1]
            + f" ({len(cumulative_rewards)} episodes)"
        )
        plt.tight_layout()
        plt.savefig(hist_fp)
        plt.close()
    ...
    def _load_episodes(self):
        ...
        for fp in tqdm(self._fps, "Loading metadata"):
            data = torch.load(fp)
            datas.append(data)
            cumrew = np.array(data["rewards"]).sum() * self._cfg.get(
                "reward_scale", 1.0
            )
        ...
        def load_episode(data, idx):
            ...
            if self._cfg.get("lazy_load", False):
                assert self._cfg.modality in {"features", "pixels"} and self._cfg.get(
                    "multitask", False
                ), f'Unexpected lazy load for modality {self._cfg.modality} and multitask={self._cfg.get("multitask", False)}'
                obs = None
            else:
                if self._cfg.modality == "features" or self._cfg.get(
                    "all_modalities", False
                ):
                    assert (
                        self._cfg.get("features", None) is not None
                    ), "Features must be specified"
                    features_dir = (
                        Path(os.path.dirname(fp)) / "features" / self._cfg.features
                    )
                    assert (
                        features_dir.exists()
                    ), "No features directory found for {}".format(fp)
                    obs = torch.load(features_dir / os.path.basename(fp))
                    _obs = np.empty(
                        (obs.shape[0], self._cfg.frame_stack * obs.shape[1]),
                        dtype=np.float32,
                    )
                ...
                if self._cfg.modality in {"pixels", "map"} or self._cfg.get(
                    "all_modalities", False
                ):
                    frames_dir = Path(os.path.dirname(fp)) / "frames"
                    assert (
                        frames_dir.exists()
                    ), "No frames directory found for {}".format(fp)
                    frame_fps = [frames_dir / fn for fn in data["frames"]]
                    obs = np.stack(
                        [np.array(Image.open(fp)) for fp in frame_fps]
                    ).transpose(0, 3, 1, 2)
            ...
            if self._cfg.get("multitask", False):
                ...
                if (
                    self._cfg.modality == "state"
                    and obs is not None
                    and obs[0].shape[0] < self._cfg.obs_shape[0]
                ):
                    obs = [
                        np.concatenate(
                            [_obs, np.zeros((self._cfg.obs_shape[0] - _obs.shape[0],))]
                        )
                        for _obs in obs
                    ]
                    ...
                    actions = np.concatenate(
                        [
                            ...
                            np.zeros(
                                (
                                    actions.shape[0],
                                    self._cfg.action_dim - actions.shape[-1],
                                )
                            ),
                        ],
                        axis=-1,
            ...
            episode = Episode.from_trajectory(
                ...
                obs,
                actions,
                np.array(data["rewards"]) * self._cfg.get("reward_scale", 1.0),
            )
            episode.info = data["infos"]
            episode.metadata = data["metadata"]
            episode.task_vec = torch.tensor(
                [
                    float(data["metadata"]["cfg"]["task"] == self._tasks[i])
        ...
        if self._cfg.get("use_val", False):
            ...
            for data, idx in tqdm(
                zip(val_datas, val_idxs), desc="Loading validation episodes"
            ):
                episode = load_episode(data, idx)
