...
class FeatureFuse(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.fn = nn.Sequential(
            nn.Linear(features_to_dim[cfg.features], cfg.enc_dim),
            nn.ELU(),
        ...
        if cfg.frame_stack == 1 and cfg.setting == "sim":
            ...
            self.layers = nn.Sequential(
                nn.ELU(),
                nn.Linear(cfg.enc_dim, cfg.mlp_dim),
                nn.ELU(),
                nn.Linear(cfg.mlp_dim, cfg.enc_dim),
                nn.ELU(),
                nn.Linear(cfg.enc_dim, cfg.latent_dim),
            )
        else:
            self.layers = nn.Sequential(
                ...
                nn.Linear(
                    cfg.enc_dim * (7 if cfg.frame_stack == 3 else 3), cfg.enc_dim
                ),
                nn.ELU(),
                nn.Linear(cfg.enc_dim, cfg.latent_dim),
            )
...
class FeatureMapEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.feature_dims[-1] == 28:
            self.layers = nn.Sequential(
                nn.Conv2d(
                    self.cfg.frame_stack * cfg.feature_dims[0],
                    cfg.num_channels,
                    3,
                    stride=2,
                ),
                nn.ReLU(),
                nn.Conv2d(cfg.num_channels, cfg.num_channels, 3, stride=1),
                nn.ReLU(),
                nn.Conv2d(cfg.num_channels, cfg.num_channels, 3, stride=1),
                nn.ReLU(),
                nn.Conv2d(cfg.num_channels, cfg.num_channels, 3, stride=1),
...
def enc(cfg):
    ...
    if cfg.modality == "map":
        ...
        out_shape = _get_out_shape(
            (cfg.frame_stack * cfg.feature_dims[0], *cfg.feature_dims[1:]), layers
        )
        ...
        if cfg.encoder == "default+":
            ...
            layers = [
                NormalizeImg(),
                nn.Conv2d(C, cfg.num_channels, 3, stride=2),
                nn.ReLU(),
                nn.Conv2d(cfg.num_channels, cfg.num_channels, 3, stride=1),
                nn.ReLU(),
                nn.Conv2d(cfg.num_channels, cfg.num_channels, 3, stride=1),
        ...
        layers = [
            nn.Linear(cfg.obs_shape[0], cfg.enc_dim),
            nn.ELU(),
            nn.Linear(cfg.enc_dim, cfg.enc_dim),
            nn.ELU(),
            nn.Linear(cfg.enc_dim, cfg.latent_dim),
        ...
        layers = [
            nn.Linear(
                cfg.obs_shape[0]
                + (cfg.latent_dim if cfg.get("multitask", False) else 0),
                cfg.enc_dim,
            ),
            nn.ELU(),
            nn.Linear(cfg.enc_dim, cfg.latent_dim),
    ...
    print(
        "Encoder parameters: {:,}".format(
            sum(p.numel() for p in nn.Sequential(*layers).parameters())
        )
    )
...
def action_enc(cfg):
    ...
    return nn.Sequential(
        nn.Linear(cfg.action_dim, cfg.enc_dim),
        nn.ELU(),
...
class ResidualBlock(nn.Module):
    def __init__(self, C):
        super().__init__()
        self.layers = nn.Sequential(
            nn.ConvTranspose2d(C, C, 3, stride=1, padding=1),
            nn.ReLU(),
            nn.ConvTranspose2d(C, C, 3, stride=1, padding=1),
            nn.ReLU(),
        )
...
def task_enc(cfg):
    ...
    if not cfg.multitask:
        return None
    return nn.Sequential(
        nn.Linear(cfg.num_tasks, cfg.enc_dim),
        nn.ELU(),
        nn.Linear(cfg.enc_dim, cfg.latent_dim),
    )
...
class Episode(object):
    ...
    def __init__(self, cfg, init_obs, init_state=None):
        ...
        self.obs = torch.empty(
            (cfg.episode_length + 1, *init_obs.shape), dtype=dtype, device=self.device
        )
...
class ReplayBuffer(object):
    ...
    def __init__(self, cfg):
        ...
        self.device = torch.device(cfg.device)
        self.capacity = (
            cfg.num_tasks if cfg.get("multitask", False) else 1
        ) * 1_000_000 + 1
        modality_to_dtype = {
            "pixels": torch.uint8,
            "map": torch.float16,
    ...
    def add(self, episode: Episode):
        ...
        if self.cfg.modality == "pixels":
            ...
            obs = episode.obs[:-1]
        if (
            self.cfg.modality == "pixels" and episode.obs.shape[1] == 3
        ):  # offline w/ frame-stacking
            last_obs = episode.obs[-self.cfg.frame_stack :].view(
                self.cfg.frame_stack * 3, *self.cfg.obs_shape[-2:]
            )
        elif (
            self.cfg.modality == "map"
            and episode.obs.shape[1] == self.cfg.feature_dims[0]
        ):  # offline w/ frame-stacking
            last_obs = episode.obs[-self.cfg.frame_stack :].view(
                self.cfg.frame_stack * self.cfg.feature_dims[0], *self.cfg.obs_shape[1:]
            )
    ...
    def _get_obs(self, arr, idxs):
        ...
        if self.cfg.modality in {"map", "pixels"}:
            obs = torch.empty(
                (self.cfg.batch_size, self.cfg.frame_stack * dim, *arr.shape[-2:]),
                dtype=arr.dtype,
    ...
    def sample(self):
        ...
        state = (
            self._state[idxs, : self._state_dim]
            if self.cfg.get("include_state", False)
            else None
        )
        next_state = (
        ...
        if state is not None:
            ...
            next_state[-1, mask] = (
                self._last_state[
                    _idxs[mask] // self.cfg.episode_length, : self._state_dim
                ]
...
class LazyReplayBufferTaskSampler(object):
    """
    Helper class that maintains sampling functionality for a single task.
    """

    def __init__(self, cfg, max_episodes, fps, task_id):
        self.cfg = cfg
    ...
    def _load_pixels(self, fp, ep_idx, idxs):
        ...
        frame_fps = [
            frames_dir / os.path.basename(fp).replace(".pt", f"_{idx:03d}.png")
            for idx in idxs
        ]
        frames = [
            self._episodes[ep_idx]["frames"].get(i, np.array(Image.open(fp)))
            for i, fp in enumerate(frame_fps)
        ]
    ...
    def _sample(self):
        ...
        if self._worker_id is None:
            ...
            if self._worker_id == 0 and self._task_id == 0:
                print(
                    f"Each worker samples from {len(self._fps)}/{total_fps} episodes per task (cache: {self.max_episodes})"
                )
        ...
        num_load = (
            ...
            if self._i == 0
                ...
                self._i % self.cfg.fetch_every == 0
                and self.max_episodes < len(self._fps)
            )
        )
        for i in range(num_load):
            fp = self._fps[np.random.randint(len(self._fps))]
            ep_idx = (
                (self._i + i) % self.max_episodes
                if num_load > 1
                else self._count.argmax()
            )
            self._episodes[ep_idx] = torch.load(fp)
            self._episodes[ep_idx]["fp"] = fp
            self._episodes[ep_idx]["frames"] = {}
            self._episodes[ep_idx]["rewards"] = np.array(
                self._episodes[ep_idx]["rewards"]
            ) * self.cfg.get("reward_scale", 1.0)
            self._count[ep_idx] = 0
            if self.cfg.modality == "features":
                feat_dir = Path(os.path.dirname(fp)) / "features" / self.cfg.features
                self._episodes[ep_idx]["features"] = torch.from_numpy(
                    torch.load(feat_dir / os.path.basename(fp))
                )
        self._i += 1
        ...
        if self.cfg.modality == "features":
            if self.cfg.frame_stack == 2:  # currently only supports 2 frames
                if idx > 0:
                    obs = torch.cat(
                        (data["features"][idx - 1], data["features"][idx]), dim=-1
                    )
                else:
                    obs = torch.cat(
                        (data["features"][idx], data["features"][idx]), dim=-1
                    )
                next_obs = torch.cat(
                    (
                        data["features"][idx : idx + self.cfg.horizon],
                        data["features"][idx + 1 : idx + self.cfg.horizon + 1],
                    ),
                    dim=-1,
                )  # .contiguous()
            ...
            obs, next_obs = self._load_pixels(
                data["fp"], ep_idx, np.arange(idx, idx + self.cfg.horizon + 1)
            )
        if self.cfg.modality == "state" or self.cfg.get("include_state", False):
            ...
            next_state = torch.from_numpy(
                np.stack(data["states"][idx + 1 : idx + self.cfg.horizon + 1])
            ).float()
        ...
        if self.cfg.get("include_state", False):
            ...
            next_state = torch.cat(
                (next_state[:, :4], next_state[:, 18 : 18 + 4]), axis=1
            )
        else:
            state, next_state = None, None
        action = (
            torch.from_numpy(np.stack(data["actions"][idx : idx + self.cfg.horizon]))
            .float()
            .clip(-1, 1)
        )
        reward = (
            torch.from_numpy(np.stack(data["rewards"][idx : idx + self.cfg.horizon]))
            .float()
            .unsqueeze(-1)
        )
...
class LazyReplayBufferIterable(IterableDataset):
    ...
    def set(self, fps, tasks):
        self._multitask = len(tasks) > 1
        self._tasks = tasks
        if self._multitask:
            self._fps = defaultdict(list)
            for fp in fps:
                for i, task in enumerate(tasks):
                    if task in fp:
                        self._fps[i].append(fp)
                        break
            self._samplers = [
                LazyReplayBufferTaskSampler(
                    self.cfg, int(self.max_episodes / len(tasks)), self._fps[i], i
                )
            ...
            self._fps = fps
            self._samplers = [
                LazyReplayBufferTaskSampler(self.cfg, self.max_episodes, self._fps, 0)
            ]
...
class LazyReplayBuffer(ReplayBuffer):
    ...
    def __init__(self, cfg):
        assert (
            cfg.frame_stack == 1 or cfg.modality == "features"
        ), "Lazy replay buffer only supports frame stacking for features"
        self.cfg = cfg
        self.capacity = cfg.get("cache_size", 1_000_000)
        self.ep_per_worker = int(
            (self.capacity / cfg.episode_length) / max(cfg.num_workers, 1)
        )
    ...
    def sample(self):
        ...
        return obs, next_obs, action, reward, state, next_state, None, None, 1.0
...
class RendererBuffer(ReplayBuffer):
    ...
    def __init__(self, cfg):
        ...
        self._pixels = torch.empty(
            (self.capacity + 1, *self.pixels_shape), dtype=torch.uint8
        )
        self._features = torch.empty(
            (self.capacity + 1, *self.features_shape), dtype=torch.float32
        )
        self._state = torch.empty(
            (self.capacity + 1, *self.state_shape), dtype=torch.float32
        )
        self._action = torch.empty((self.capacity, cfg.action_dim), dtype=torch.float32)
        self._last_pixels = torch.empty(
            (
                self.capacity // cfg.episode_length,
                3 * self.cfg.frame_stack,
                *self.pixels_shape[-2:],
            ),
            dtype=torch.uint8,
        )
        self._last_features = torch.empty(
            (self.capacity // cfg.episode_length, *self.features_shape),
            dtype=torch.float32,
        )
        self._last_state = torch.empty(
            (self.capacity // cfg.episode_length, *self.state_shape),
            dtype=torch.float32,
        )
    ...
    def add(self, episode: Episode):
        assert not self.full, "Replay buffer is full"
        self._pixels[self.idx : self.idx + self.cfg.episode_length] = torch.from_numpy(
            episode.metadata["pixels"][:-1, -3:]
        )
        self._features[
            self.idx : self.idx + self.cfg.episode_length
        ] = torch.from_numpy(episode.metadata["features"][:-1])
        self._state[self.idx : self.idx + self.cfg.episode_length] = torch.from_numpy(
            np.array(episode.metadata["states"][:-1])
        )
        self._action[self.idx : self.idx + self.cfg.episode_length] = episode.action
        self._last_pixels[self.idx // self.cfg.episode_length] = torch.from_numpy(
            episode.metadata["pixels"][-self.cfg.frame_stack :]
        ).reshape(3 * self.cfg.frame_stack, *self.pixels_shape[-2:])
        self._last_features[self.idx // self.cfg.episode_length] = torch.from_numpy(
            episode.metadata["features"][-1]
        )
        self._last_state[self.idx // self.cfg.episode_length] = torch.from_numpy(
            episode.metadata["states"][-1]
        )
        mask = (
            torch.arange(self.cfg.episode_length)
            >= self.cfg.episode_length - self.cfg.horizon
        )
        new_priorities = torch.full((self.cfg.episode_length,), 1.0)
        new_priorities[mask] = 0
        self._priorities[self.idx : self.idx + self.cfg.episode_length] = new_priorities
        self.idx = (self.idx + self.cfg.episode_length) % self.capacity
        self._full = self._full or self.idx == 0
    ...
    def sample(self, modalities):
        ...
        idxs = torch.from_numpy(
            np.random.choice(
                total, self.cfg.batch_size, p=probs.cpu().numpy(), replace=False
            )
        )

        if "pixels" in modalities:
            pixels = self._get_pixel_obs(self._pixels, idxs)
            next_pixels = torch.empty(
                (
                    self.cfg.horizon + 1,
                    self.cfg.batch_size,
                    3 * self.cfg.frame_stack,
        ...
        if "features" in modalities:
            ...
            next_features = torch.empty(
                (self.cfg.horizon + 1, self.cfg.batch_size, *self.features_shape),
                dtype=torch.float32,
            )
        if "state" in modalities:
            state = self._state[idxs]
            next_state = torch.empty(
                (self.cfg.horizon + 1, self.cfg.batch_size, *self.state_shape),
                dtype=torch.float32,
            )
        action = torch.empty(
            (self.cfg.horizon + 1, self.cfg.batch_size, self.cfg.action_dim),
            dtype=torch.float32,
        )
