...
class MultiTOLD(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self._dynamics = h.mlp(
            cfg.latent_dim + cfg.action_emb_dim, 2 * cfg.mlp_dim, cfg.latent_dim
        )
        self._heads = nn.ModuleList(
            [TaskTOLD(cfg, task_id) for task_id in range(cfg.num_tasks)]
        )
...
class MultiTDMPC:
    ...
    def __init__(self, cfg):
        ...
        self.pi_optims = [
            torch.optim.Adam(self.model._heads[i]._pi.parameters(), lr=self.cfg.lr)
    ...
    def state_dict(self):
        ...
        return {
            ...
            "pi_optims": [
                self.pi_optims[i].state_dict() for i in range(self.cfg.num_tasks)
            ],
        }
    ...
    def estimate_value(self, z, actions, task_vec, horizon):
        ...
        G += discount * torch.min(
            *self.model.Q(z, self.model.pi(z, task_vec, self.cfg.min_std), task_vec)
        )
        return G

    @torch.no_grad()
    def plan(
        self, obs, task_vec, state, eval_mode=False, step=None, t0=True, open_loop=False
    ):
        """
        ...
        if step < self.cfg.seed_steps and not eval_mode:
            return torch.empty(
                self.cfg.action_dims[task_id], dtype=torch.float32, device=self.device
            ).uniform_(-1, 1)
        ...
        if num_pi_trajs > 0:
            pi_actions = torch.empty(
                horizon, num_pi_trajs, self.cfg.action_dims[task_id], device=self.device
            )
    ...
    def update_pi(self, zs, task_id):
        ...
        torch.nn.utils.clip_grad_norm_(
            self.model._heads[task_id]._pi.parameters(),
    ...
    def _td_target(self, next_obs, next_state, reward, task_id):
        ...
        td_target = reward + self.cfg.discount * torch.min(
            *self.model_target.Q(
                next_z, self.model.pi(next_z, task_id, self.cfg.min_std), task_id
            )
        )
        return td_target
    ...
    def update(self, replay_buffer, step=int(1e6)):
        ...
        for task_id in range(self.cfg.num_tasks):
            ...
            for t in range(self.cfg.horizon):
                ...
                with torch.no_grad():
                    ...
                    next_state = (
                        next_states[task_id, t] if next_states is not None else None
                    )
                    next_z = self.model_target.h(next_obs, next_state)
                    td_target = self._td_target(
                        next_obs, next_state, reward[task_id, t], task_id
                    )
        ...
        weighted_loss.register_hook(
            lambda grad: grad * (1 / (self.cfg.horizon * self.cfg.num_tasks))
        )
