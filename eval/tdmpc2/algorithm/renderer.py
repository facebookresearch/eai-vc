...
class StateToPixels(nn.Module):
    ...
    def __init__(self):
        ...
        self.load_state_dict(
            torch.load("/private/home/nihansen/code/tdmpc2/recon/renderer_27000.pt")[
                "model"
            ]
        )
...
class Renderer:
    ...
    def preprocess_target(self, target):
        if self.cfg.target_modality == "pixels":
            return self._resize(target[:, -3:] / 255.0).clip(0, 1)
        return target
    ...
    def update(self, buffer):
        ...
        target = torch.cat(
            (
                dictionary[self.cfg.target_modality].unsqueeze(0),
                dictionary["next_" + self.cfg.target_modality],
            ),
            dim=0,
        )
        ...
        for t in range(self.cfg.horizon):
            total_loss += (self.cfg.rho**t) * h.mse(
                self.decoder(z), self.preprocess_target(target[t])
            ).mean()
