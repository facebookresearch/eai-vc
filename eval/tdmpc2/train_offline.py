...
def evaluate(env, agent, cfg, iteration, video):
    ...
    for i in range(cfg.eval_episodes):
        ...
        if video:
            video.save(iteration, f"videos/{env.task}") if cfg.get(
                "multitask", False
            ) else video.save(iteration)
    episode_rewards = np.array(episode_rewards)
    episode_successes = np.array(episode_successes)
    return (
        np.nanmean(episode_rewards),
        episode_rewards,
        np.nanmean(episode_successes),
        episode_successes,
    )
...
def train_offline(cfg: dict):
    ...
    try:
        print(
            f"Buffer contains {buffer.capacity if buffer.full else buffer.idx} transitions, capacity is {buffer.capacity-1}"
        )
    ...
    for iteration in range(common_metrics["iteration"], cfg.train_iter + 1):
        ...
        if iteration % cfg.eval_freq == 0:
            ...
            if cfg.get("multitask", False):
                task_idxs = np.array(
                    [i % cfg.num_tasks for i in range(cfg.eval_episodes)]
                )
                task_rewards = np.empty(
                    (cfg.num_tasks, cfg.eval_episodes // cfg.num_tasks)
                )
                task_successes = np.empty(
                    (cfg.num_tasks, cfg.eval_episodes // cfg.num_tasks)
                )
                ...
                common_metrics.update(
                    {
                        f"task_reward/{task}": task_rewards[i]
                        for i, task in enumerate(env.tasks)
                    }
                )
                common_metrics.update(
                    {
                        f"task_success/{task}": task_successes[i]
                        for i, task in enumerate(env.tasks)
                    }
                )
            else:
