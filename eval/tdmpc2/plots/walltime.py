...
def main():
    ...
    exp_names = [
        ...
        "offline-v1",
        "mocodmcontrol-v1",
        "mocodmcontrol-offline-v1",
        "mocometaworld-v1",
        "mocometaworld-offline-v1",
        "mocoego-v1",
        "mocoego-offline-v1",
        "random-v1",
        "random-offline-v1",
    ...
    for run in runs:
        ...
        print(
            f"Appending experiment {label} with walltime {round(walltime)}s per 100k steps"
        )
        entries.append((idx, cfg["task"], label, seed, walltime))

    df = pd.DataFrame(
        entries, columns=["idx", "task", "experiment", "seed", "walltime"]
    )
