...
def main():
    ...
    exp_names = [
        ...
        "offline-v2-fs",
        "mocodmcontrol-v1",
        "mocodmcontrol-offline-v2",
        "mocometaworld-v1",
        "mocometaworld-offline-v2-fs",
        "mocoego-v1",
        "mocoego-offline-v2",
        "mocoego-offline-v2-fs",
        "random-v1",
        "random-offline-v2",
        "bc-offline-v2",
    ]
    ...
    runs = api.runs(
        ...
        filters={
            "$or": [
                {"tags": task} for task in (tasks["dmcontrol"] + tasks["metaworld"])
            ],
    ...
    for run in runs:
        ...
        if (
            task not in (tasks["dmcontrol"] + tasks["metaworld"])
            or exp_name not in exp_names
            or seed not in seeds
        ):
            continue
        if (
            exp_name == "mocoego-offline-v2"
            or (exp_name == "offline-v2" and modality == "pixels")
        ) and task in tasks["metaworld"]:
            continue
    ...
    df_dmcontrol = (
        df_dmcontrol.groupby(["idx", "experiment", "seed"]).mean().reset_index()
    )
    df_metaworld = (
        df_metaworld.groupby(["idx", "experiment", "seed"]).mean().reset_index()
    )
    ...
    df_dmcontrol["reward"] = (
        100
        * df_dmcontrol["reward"]
        / df_dmcontrol[df_dmcontrol["experiment"] == "State"]["reward"].values[0]
    ).round()
    df_metaworld["reward"] = (
        100
        * df_metaworld["reward"]
        / df_metaworld[df_metaworld["experiment"] == "State"]["reward"].values[0]
    ).round()
    ...
    ax = axs[0]
    sns.barplot(
        data=df_dmcontrol, x="experiment", y="reward", ax=ax, ci=None, palette=colors
    )
    ...
    ax = axs[1]
    sns.barplot(
        data=df_metaworld, x="experiment", y="reward", ax=ax, ci=None, palette=colors
    )
