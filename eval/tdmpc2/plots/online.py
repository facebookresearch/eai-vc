...
def main():
    ...
    for run in runs:
        ...
        for i in range(len(step)):
            entries.append((idx, cfg["task"], label, seed, step[i], reward[i]))

    df = pd.DataFrame(
        entries, columns=["idx", "task", "experiment", "seed", "step", "reward"]
    )
    ...
    df_dmcontrol = (
        df_dmcontrol.groupby(["idx", "experiment", "seed", "step"]).mean().reset_index()
    )
    df_metaworld = (
        df_metaworld.groupby(["idx", "experiment", "seed", "step"]).mean().reset_index()
    )
