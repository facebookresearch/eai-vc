...
def main():
    ...
    df = pd.DataFrame(
        entries, columns=["idx", "task", "experiment", "horizon", "seed", metric]
    )
    ...
    # plot
    g = sns.barplot(
        data=df, x="experiment", y=metric, hue="horizon", ax=ax, ci=None, palette=colors
    )
    g.legend_.remove()
