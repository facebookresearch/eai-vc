...
from trifinger_mbirl.forward_models.models.forward_model import get_obs_vec_from_obs_dict
...
def get_traj_dict_from_obs_list(data, scale=1, include_image_obs=True):
    ...
    if("object_observation" in data_keys): #prev. key in obs
        ...
        robot_pos = np.array([data[i]["robot_observation"]["position"] for i in range(len(data))])
        ...
        position_error = np.array([data[i]["achieved_goal_position_error"] for i in range(len(data))])
...
def get_grad_cam(r3m_model, forward_model, img_cur, img_next, obs_dict):
        ...
        cam_pred = GradCAM(model=model,
                        target_layers=target_layers,
                        use_cuda=False)

        cam_gt = GradCAM(model=r3m_model,
        ...
        img_cur_preproc = transforms(Image.fromarray(img_cur.astype(np.uint8))).reshape(-1, 3, 224, 224)
        img_next_preproc = transforms(Image.fromarray(img_next.astype(np.uint8))).reshape(-1, 3, 224, 224)
        ...
        grayscale_hm_pred = cam_pred(input_tensor=input_tensor, targets=None) # [input_tensor.shape]
        ...
        visualization_pred = show_cam_on_image(img_next_postproc, grayscale_hm_pred, use_rgb=True)

        grayscale_hm_gt = grayscale_hm_gt[0, :]
        visualization_gt = show_cam_on_image(np.float32(img_next_preproc[0]).transpose(1, 2, 0), grayscale_hm_gt ,use_rgb=True)

        return visualization_pred, visualization_gt

def resize_img(img, new_dim=64):

    resize = T.Compose([T.Resize(new_dim, interpolation=T.InterpolationMode.BICUBIC),
                        T.ToTensor()])
...
def get_obs_dict_from_traj(traj, t, obj_state_type):
    ...
    if "mode" in traj: obs_dict["mode"] = traj["mode"][t]

    if obj_state_type == "na": return obs_dict # No object state
