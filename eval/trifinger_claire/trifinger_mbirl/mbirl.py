...
class IRLLoss(object):
    def __init__(self, irl_loss_state, obj_state_type, mpc_use_ftpos):
        ...
        self.mpc_use_ftpos = (
            mpc_use_ftpos  # True if forward model includes ftpos in state
        )

    def __call__(self, full_pred_traj, expert_demo_dict):
        pred_traj = d_utils.parse_pred_traj(
            full_pred_traj, self.irl_loss_state, mpc_use_ftpos=self.mpc_use_ftpos
        )
        target_traj = get_expert_demo(
            expert_demo_dict, self.irl_loss_state, self.obj_state_type
        )
...
class MBIRL:
    def __init__(self, conf, traj_info):
        ...
        log.info(
            f"Loaded MPC type {self.conf.mpc_type} with obj_state_type = {self.mpc.obj_state_type}"
        )

        # Ensure mpc_type and cost/irl_loss states are compatible
        if conf.mpc_type == "learned" and (
            "ftpos" in conf.cost_state or "ftpos" in conf.irl_loss_state
        ):
            assert (
                mpc.use_ftpos
            ), "Forward model does not use ftpos, but cost_state or irl_loss_state requires ftpos"

        self.mpc_use_ftpos = True
        if conf.mpc_type == "learned" and not self.mpc.use_ftpos:
            self.mpc_use_ftpos = False

        # IRL loss
        self.irl_loss_fn = IRLLoss(
            conf.irl_loss_state, self.mpc.obj_state_type, self.mpc_use_ftpos
        )

        # Get learnable cost function
        self.learnable_cost = get_learnable_cost(
            conf, time_horizon, self.mpc.obj_state_type
        )
    ...
    def train(self, model_data_dir=None, no_wandb=False):
        ...
        learnable_cost_opt = torch.optim.Adam(
            self.learnable_cost.parameters(), lr=self.conf.cost_lr
        )
        ...
        for outer_i in range(self.conf.n_outer_iter):
            ...
            for demo_i in range(len(train_trajs)):
                ...
                obs_dict_init = d_utils.get_obs_dict_from_traj(
                    expert_demo_dict, 0, self.mpc.obj_state_type
                )  # Initial state
                ...
                action_optimizer = torch.optim.SGD(
                    self.mpc.parameters(), lr=self.conf.action_lr
                )

                with higher.innerloop_ctx(self.mpc, action_optimizer) as (
                    fpolicy,
                    diffopt,
                ):
                    for i in range(self.conf.n_inner_iter):
                        ...
                        pred_traj_for_cost = d_utils.parse_pred_traj(
                            pred_traj,
                            self.conf.cost_state,
                            mpc_use_ftpos=self.mpc_use_ftpos,
                        )
                        learned_cost_val = self.learnable_cost(
                            pred_traj_for_cost, target
                        )
                ...
                if (outer_i + 1) % self.conf.n_epoch_every_log == 0:
                    ...
                    traj_plots_dir = os.path.join(
                        plots_dir, f"diff-{diff}_traj-{traj_i}"
                    )
                    if not os.path.exists(traj_plots_dir):
                        os.makedirs(traj_plots_dir)
                    ...
                    if self.conf.mpc_type == "learned" and self.decoder is not None:
                        ...
                        with torch.no_grad():  # TODO need to freeze decoder weights??
                            ...
                            self.decoder.save_gif(
                                pred_imgs,
                                os.path.join(
                                    traj_plots_dir, f"r3m_epoch_{outer_i+1}.gif"
                                ),
                            )

                    pred_actions = fpolicy.action_seq.data.detach().numpy()
                    plot_actions(
                        traj_plots_dir, outer_i, pred_actions, expert_demo_dict
                    )
            ...
            (
                test_irl_losses,
                test_pred_trajs,
                test_pred_actions_per_demo,
            ) = self.evaluate_action_optimization(
                test_trajs, plots_dir=test_dir_name, outer_i=outer_i
            )
    ...
    def evaluate_action_optimization(self, trajs, plots_dir=None, outer_i=None):
        ...
        for t_i, expert_demo_dict in enumerate(trajs):
            ...
            for i in range(self.conf.n_inner_iter):
                ...
                target = self.get_target_for_cost_type(expert_demo_dict)
                pred_traj_for_cost = d_utils.parse_pred_traj(
                    pred_traj, self.conf.cost_state, mpc_use_ftpos=self.mpc_use_ftpos
                )
                learned_cost_val = self.learnable_cost(
                    pred_traj_for_cost, target
                )  # TODO check that correct cost is being used
            ...
            eval_costs.append(
                self.irl_loss_fn(pred_state_traj_new, expert_demo_dict).mean()
            )
            ...
            if plots_dir is not None:
                ...
                if self.conf.mpc_type == "learned":
                    ft_states, o_states = self.mpc.get_states_from_x_next(
                        pred_state_traj_new
                    )
                    with torch.no_grad():  # TODO need to freeze decoder weights??
                        pred_imgs = self.decoder(torch.Tensor(o_states))
                        self.decoder.save_gif(
                            pred_imgs,
                            os.path.join(traj_plots_dir, f"r3m_epoch_{outer_i+1}.gif"),
                        )
...
def get_mpc(mpc_type, time_horizon, mpc_forward_model_ckpt=None):
    ...
    if mpc_type == "ftpos":
        ...
        if mpc_forward_model_ckpt is None:
            raise ValueError("Missing mpc_forward_model_ckpt")
        ...
        if "learned_only" in mpc_type:
            learned_only = True
        else:
            learned_only = False
        return TwoPhaseMPC(
            time_horizon - 1, mpc_forward_model_ckpt, learned_only=learned_only
        )
...
def plot_traj(
    plots_dir, outer_i, pred_traj, expert_demo_dict, mpc_type, obj_state_type
):
    ...
    if "ftpos_obj" in mpc_type and obj_state_type == "pos":  # TODO hardcoded
        ...
        plot_expert_demo = get_expert_demo(
            expert_demo_dict, "ftpos_obj", obj_state_type
        )
    else:
    ...
    d_utils.plot_traj(
        ...
        d_list,
        {
            ...
            "demo": {
                "y": plot_expert_demo.detach().numpy(),
                "x": expert_demo_dict["t"],
                "marker": ".",
            },
        },
...
def get_expert_demo(traj_dict, expert_demo_state, obj_state_type):
    ...
    if expert_demo_state == "ftpos":
        ...
        expert_demo = torch.cat(
            [torch.Tensor(traj_dict["ft_pos_cur"]), torch.Tensor(o_expert_demo)], dim=1
        )
