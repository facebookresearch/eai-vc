...
def train(
    ...
    for epoch in range(conf.algo.n_epochs):
        ...
        for i_batch, batch in enumerate(train_dataloader):
            ...
            obs, gt_state_next = get_obs_and_state_next_from_batch(
                batch, use_ftpos=conf.algo.use_ftpos
            )
                ...
                for one_step in [False, True]:
                    ...
                    for i, traj in enumerate(traj_list):
                        ...
                        if diff in plot_count_dict:
                            if plot_count_dict[diff] >= MAX_PLOT_PER_DIFF:
                                continue
                            else:
                        ...
                        traj_dir = os.path.join(
                            plots_dir, split_name, f"diff-{diff}_traj-{traj_i}"
                        )
                        ...
                        pred_traj_dict, pred_err_dict = test_mpc(
                            mpc, traj, epoch + 1, save_dir=traj_dir, one_step=one_step
                        )

                        max_ft_pos_l2_dist = max(
                            pred_err_dict["max_ft_pos_l2_dist"], max_ft_pos_l2_dist
                        )
                        max_o_state_l2_dist = max(
                            pred_err_dict["max_o_state_l2_dist"], max_o_state_l2_dist
                        )
                        ...
                        if decoder is not None:
                            ...
                            decoder.save_gif(
                                pred_imgs,
                                os.path.join(
                                    traj_dir, f"{save_label}_epoch_{epoch+1}.gif"
                                ),
                            )

                    loss_dict[
                        f"{split_name}_{save_label}_max_ft_pos_l2_dist"
                    ] = max_ft_pos_l2_dist
                    loss_dict[
                        f"{split_name}_{save_label}_max_o_state_l2_dist"
                    ] = max_o_state_l2_dist
                    loss_dict[
                        f"{split_name}_{save_label}_avg_ft_pos_l2_dist"
                    ] = avg_ft_pos_l2_dist / len(traj_list)
                    loss_dict[
                        f"{split_name}_{save_label}_avg_o_state_l2_dist"
                    ] = avg_o_state_l2_dist / len(traj_list)
...
def main(conf):
    ...
    train_dataset = ForwardModelDataset(
        train_trajs, obj_state_type=conf.algo.obj_state_type, device=device
    )
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=16, shuffle=True
    )

    test_dataset = ForwardModelDataset(
        test_trajs, obj_state_type=conf.algo.obj_state_type, device=device
    )
    ...
    if conf.algo.use_ftpos:
        in_dim = (
            train_dataset.a_dim + train_dataset.o_state_dim + train_dataset.ft_state_dim
        )
