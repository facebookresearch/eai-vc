    ...
    for epoch in range(conf.algo.n_epochs):
        ...
        if (epoch + 1) % conf.algo.n_epoch_every_save == 0:
            ...
            for split_name, dataloader in [
                ["train", train_dataloader_traj_order],
                ...
                for batch in dataloader:
                    ...
                    model.save_gif(
                        pred_imgs,
                        os.path.join(plots_dir, traj_dir, f"epoch_{epoch+1}.gif"),
                    )
...
def main(conf):
    ...
    train_dataset = DecoderDataset(
        train_trajs, traj_info["train_demo_stats"], device=device
    )
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=16, shuffle=True
    )
    train_dataloader_traj_order = torch.utils.data.DataLoader(
        train_dataset, batch_size=20, shuffle=False
    )

    test_dataset = DecoderDataset(
        test_trajs, traj_info["test_demo_stats"], device=device
    )
