...
def test_mpc(mpc, traj, epoch, save_dir=None, one_step=False):
    ...
    pred_err_dict = get_pred_traj_err(pred_traj_dict, traj, mpc.obj_state_type, epoch, save_dir=save_dir, one_step=one_step)

    return pred_traj_dict, pred_err_dict

def get_pred_traj_err(pred_traj, gt_traj, obj_state_type, epoch, save_dir=None, one_step=False):
    """
    ...
    if pred_ft_states is not None:
        for i in range(3):
            per_finger_err = np.linalg.norm((pred_ft_states[:, i*3:i*3+3] - gt_traj["ft_pos_cur"][:, i*3:i*3+3]), axis=1)
            ft_pos_err[:, i] = per_finger_err

        if save_dir is not None:
            # Plot L2 distance between each predicted ftpos and gt ftpos
            save_str = os.path.join(save_dir, f"{save_label}_ftpos_l2_dist_epoch_{epoch}.png")
            d_utils.plot_traj(
    ...
    if save_dir is not None:
        save_str = os.path.join(save_dir, f"{save_label}_o_state_l2_dist_epoch_{epoch}.png")
    else:
        save_str = None

    if obj_state_type == "pos":
        o_state_err = np.expand_dims(np.linalg.norm((pred_o_states - true_o_state), axis=1), 1)
        ...
        for i in range(8):
            per_vert_err = np.linalg.norm((pred_o_states[:, i*3:i*3+3] - true_o_state[:, i*3:i*3+3]), axis=1)
...
def main(args):
    ...
    epoch = os.path.basename(args.model_path).split("_")[1] # TODO get epoch from ckpt path
    ...
    for split_name in ["train", "test"]:
        for i, traj in enumerate(traj_info[f"{split_name}_demos"]):
            ...
            for one_step in [False, True]:
                pred_traj_dict, pred_err_dict = test_mpc(mpc, traj, epoch, one_step=one_step)
                pred_imgs = decoder(torch.Tensor(pred_traj_dict["o_state"]))

                save_label = "pred_one_step" if one_step else "pred_rollout"
                decoder.save_gif(pred_imgs, os.path.join(traj_plots_dir, f'{save_label}_epoch_{epoch}.gif'))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_path", "-m", default=None, help="""Filepath of model ckpt to load""")
    parser.add_argument("--decoder_model_path", "-d", default=None, help="""Filepath of model ckpt to load""")
