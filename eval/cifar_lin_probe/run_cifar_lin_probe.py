...
def probe_model_eval(config):
    ...
    probe = torch.nn.Sequential(
        torch.nn.BatchNorm1d(embedding_dim), torch.nn.Linear(embedding_dim, 10)
    )

    raw_tr_data = torchvision.datasets.CIFAR10(
        root=config["data_root"], train=True, download=True, transform=transform
    )
    raw_te_data = torchvision.datasets.CIFAR10(
    ...
    trainloader = torch.utils.data.DataLoader(
        tr_data, batch_size=config["batch_size"], shuffle=True, num_workers=2
    )
    testloader = torch.utils.data.DataLoader(
        te_data, batch_size=config["batch_size"], shuffle=False, num_workers=2
    )
    ...
    for epoch in range(config["num_epochs"]):
        ...
        print("===========================")
        print(
            f"epoch : %i | train accuracy : %3.2f | test accuracy : % 3.2f"
