...
def load_and_save_df(top_dir):
    ...
    for item_name in os.listdir(top_dir):
        ...
        if not os.path.exists(top_eval_dir):
            continue  # skip directory if it doesn't contain eval/
        ...
        for train_or_test in ["test", "train"]:
            ...
            for traj_num in range(n_demos):
                ...
                if algo == "mbirl":
                    test_loss = (
                        ckpt_info[f"irl_loss_{train_or_test}_per_demo"][traj_num]
                        .detach()
                        .item()
                    )
                else:
                ...
                final_pos_err = data[-1]["achieved_goal"][
                    "position_error"
                ...
                data_dict["final_pos_err"].append(
                    expert_demo["position_error"][-1] / scale
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "top_dir",
        default=None,
        help="""Filepath of top-level directory containing experiment directories""",
    )
