...
class TwoPhaseMPC(torch.nn.Module):
    def __init__(
        self, time_horizon, phase2_model_path, f_num=3, mode=None, learned_only=False
    ):
        super().__init__()
    ...
    def roll_out(self, obs_dict_init):
        ...
        if self.mode is None:
            if t_ind < self.phase2_start_ind:
                mode = 1
            else:
                mode = 2
        else:
...
def main(args):
    ...
    for i in range(3):
        per_finger_err = np.linalg.norm(
            (
                pred_ft_pos[:, i * 3 : i * 3 + 3]
                - traj["ft_pos_cur"][:, i * 3 : i * 3 + 3]
            ),
            axis=1,
        )
    ...
    if mpc.obj_state_type == "pos":
        o_state_err = np.expand_dims(
            np.linalg.norm((pred_o_state - true_o_state), axis=1), 1
        )
    elif mpc.obj_state_type == "vertices":
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-m",
        default=None,
        help="""Filepath of phase2 model to load""",
    )
    parser.add_argument(
        "--demo_path", "-d", default=None, help="""Filepath of demo to load for test"""
    )
