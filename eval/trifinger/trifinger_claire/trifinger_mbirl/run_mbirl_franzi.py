...
def evaluate_action_optimization(
    conf, learned_cost, irl_loss_fn, trajs, plots_dir=None
):
...
def irl_training(
    ...
    for outer_i in range(n_outer_iter):
        ...
        test_irl_losses = evaluate_action_optimization(
            conf,
            learnable_cost.eval(),
            irl_loss_fn,
            test_trajs,
        ...
        print(
            "irl loss (on test) training iter: {} loss: {}".format(
                outer_i + 1, test_irl_losses.mean().item()
            )
        )
        print("")
    ...
    return (
        torch.stack(irl_loss_on_train),
        torch.stack(irl_loss_on_test),
        learnable_cost_params,
        pred_traj,
    )
...
def main(conf):
    ...
    train_trajs = [traj]
    test_trajs = [traj]
    irl_loss_train, irl_loss_test, learnable_cost_params, pred_traj = irl_training(
        conf,
        learnable_cost,
        irl_loss_fn,
        train_trajs,
