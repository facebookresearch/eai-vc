...
class PolicyOpt:
    def __init__(self, conf, traj_info, device):
        ...
        if conf.mpc_type == "learned" and ("ftpos" in conf.cost_state):
            assert (
                mpc.use_ftpos
            ), "Forward model does not use ftpos, but cost_state requires ftpos"
    ...
    def train(self, model_data_dir=None, no_wandb=False):
        ...
        for demo_i in range(len(train_trajs)):
            ...
            expert_actions = torch.Tensor(expert_demo_dict["delta_ftpos"][:-1]).to(
                self.device
            )
            ...
            for outer_i in range(5):
                ...
                for inner_i in range(self.conf.n_inner_iter):
                    ...
                    if (inner_i + 1) % self.conf.n_epoch_every_log == 0:
                        ...
                        pred_actions = (
                            self.mpc.action_seq.clone().data.cpu().detach().numpy()
                        )
                        self.plot(
                            traj_plots_dir,
                            inner_i + 1,
                        ...
                        rollout_save_path = os.path.join(
                            sim_dir, f"{traj_label}_epoch_{inner_i+1}.npz"
                        )
                        pred_traj_sim = self.sim.rollout_actions(
                            expert_demo_dict, pred_actions, save_path=rollout_save_path
                        )
                        # Save gif of sim rollout
                        d_utils.save_gif(
                            pred_traj_sim["image_60"],
                            os.path.join(
                                sim_dir, f"viz_{traj_label}_epoch_{inner_i+1}.gif"
                            ),
                        )
                        ...
                        sim_err = np.linalg.norm(
                            pred_traj_sim["image_60_r3m"]
                            - pred_traj.cpu().detach().numpy()
                        )
                ...
                if self.conf.train_forward_model:
                    ...
                    self.mpc.train_forward_model(
                        new_traj_list,
                        self.conf.forward_model_n_epochs,
                        sim_dir,
                        no_wandb=no_wandb,
                    )
        ...
        if self.conf.mpc_type == "learned" and self.decoder is not None:
            ...
            with torch.no_grad():  # TODO need to freeze decoder weights??
                ...
                self.decoder.save_gif(
                    pred_imgs, os.path.join(traj_plots_dir, f"r3m_epoch_{outer_i}.gif")
                )
...
def get_mpc(mpc_type, time_horizon, device, mpc_forward_model_ckpt=None):
    ...
    if mpc_type == "ftpos":
        ...
        return LearnedMPC(time_horizon - 1, model_dict=model_dict, device=device).to(
            device
        )
