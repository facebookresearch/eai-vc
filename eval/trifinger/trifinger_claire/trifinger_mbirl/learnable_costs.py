...
class RBFWeights(torch.nn.Module):
    ...
    def __init__(self, time_horizon, dim, width, kernels=5, weights=None):
        ...
        k_list = torch.linspace(0, time_horizon-1, kernels) # 5 here is # of kernels (J in mbirl paper)
        if weights is None:
...
class LearnableRBFWeightedCost(torch.nn.Module):
    def __init__(self, time_horizon, dim=9, width=2.0, kernels=5, weights=None):
        ...
        self.dim = dim
        self.weights_fn = RBFWeights(time_horizon=time_horizon, dim=dim, width=width, kernels=kernels,
                                     weights=weights)
...
class LearnableMultiPhaseWeightedCost(torch.nn.Module):
    ...
    def forward(self, y_in, y_target_per_mode):
        ...
        """
        assert y_in.dim() == 2
        assert y_target_per_mode.shape[0] == self.mode, "Number of targets does not equal number of modes"

        time_horizon = y_in.shape[0]
        total_errors = torch.zeros([time_horizon, self.dim])

        for i in range(self.mode):
            sqrd_dist_to_target_i = (y_in - y_target_per_mode[i, :]) ** 2 # [time_horizon, dim]
