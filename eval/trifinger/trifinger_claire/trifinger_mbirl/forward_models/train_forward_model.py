...
from trifinger_mbirl.forward_models.datasets.forward_model_dataset import (
    ForwardModelDataset,
)
...
# TODO: deal with MPC, wandb logging when training forward model online


class ForwardModelTrainer:
    def __init__(self, model=None, conf=None, device="cpu"):
        self.conf = conf
        self.device = device
        ...
        self.traj_info = torch.load(conf.demo_path)
        train_trajs = self.traj_info["train_demos"]
        test_trajs = self.traj_info["test_demos"]

        # Datasets and dataloaders
        self.train_dataset = ForwardModelDataset(
            train_trajs,
            obj_state_type=self.conf.algo.obj_state_type,
            device=self.device,
        )
        self.test_dataset = ForwardModelDataset(
            test_trajs, obj_state_type=self.conf.algo.obj_state_type, device=self.device
        )

        if model is None:
            if conf.algo.use_ftpos:
                in_dim = (
                    self.train_dataset.a_dim
                    + self.train_dataset.o_state_dim
                    + self.train_dataset.ft_state_dim
                )
                out_dim = (
                    self.train_dataset.o_state_dim + self.train_dataset.ft_state_dim
                )
            else:
                in_dim = self.train_dataset.a_dim + self.train_dataset.o_state_dim
                out_dim = self.train_dataset.o_state_dim
            hidden_dims = list(
                map(int, str(conf.algo.hidden_dims).split("-"))
            )  # d1-d2 (str) --> [d1, d2] (list of ints)
            self.model = ForwardModel(in_dim, out_dim, hidden_dims).to(self.device)
        else:
            self.model = model

        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=conf.algo.lr)
    ...
    def update_data(self, new_traj_list):
        # Append new_traj to train_dataset
        for demo in new_traj_list:
            self.train_dataset.add_new_traj(demo)
        log.info(f"Updated training dataset. New length: {len(self.train_dataset)}")

    def train(self, n_epochs, model_data_dir, no_wandb=True):
        ...
        for epoch in range(n_epochs):
            ...
            for i_batch, batch in enumerate(train_dataloader):
                ...
                obs, gt_state_next = self.get_obs_and_state_next_from_batch(batch)

                self.optimizer.zero_grad()
                pred_state_next = self.model(obs)
                loss = self.loss_fn(pred_state_next, gt_state_next)
                loss.backward()
                self.optimizer.step()
            ...
            with torch.no_grad():
                for i_batch, batch in enumerate(test_dataloader):
                    obs, gt_state_next = self.get_obs_and_state_next_from_batch(batch)
                    pred_state_next = self.model(obs)
                    test_loss = self.loss_fn(pred_state_next, gt_state_next)
                    total_test_loss += test_loss.item()
            avg_test_loss = total_test_loss / (i_batch + 1)
            print(f"Epoch: {epoch}, test loss: {avg_test_loss}")

            loss_dict = {
                "forward_model_train_loss": avg_train_loss,
                "forward_model_test_loss": avg_test_loss,
                "forward_model_epoch": epoch,
                "train_dataset_len": len(self.train_dataset),
            }

            if (epoch + 1) % self.conf.algo.n_epoch_every_save == 0:
                # Save checkpoint
                model_dict = {
                    "loss_train": loss,
            ...
            if not no_wandb:
                t_utils.plot_loss(loss_dict)

    def get_obs_and_state_next_from_batch(self, batch):

        obs = get_obs_vec_from_obs_dict(
            batch["obs"], use_ftpos=self.conf.algo.use_ftpos, device=self.device
        )

        if self.conf.algo.use_ftpos:
            gt_state_next = torch.cat(
                [batch["state_next"]["ft_state"], batch["state_next"]["o_state"]], dim=1
            )
        else:
...
def main(conf):
    assert (
        conf.algo.name == "forward_model"
    ), "Need to use algo=forward_model when running script"
    ...
    trainer = ForwardModelTrainer(conf=conf, device=device)
    log.info(f"Model:\n{trainer.model}")

    trainer.train(conf.algo.n_epochs, exp_dir, no_wandb=conf.no_wandb)
