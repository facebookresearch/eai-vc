...
class ForwardModelTrainer:
    ...
    def __init__(self, model=None, conf=None, device="cpu"):
        ...
        self.train_dataset = ForwardModelDataset(train_trajs, obj_state_type=self.conf.algo.obj_state_type, device=self.device)
        self.test_dataset = ForwardModelDataset(test_trajs, obj_state_type=self.conf.algo.obj_state_type, device=self.device)

        if model is None:
            if conf.algo.use_ftpos:
                in_dim = self.train_dataset.a_dim + self.train_dataset.o_state_dim + self.train_dataset.ft_state_dim
                out_dim = self.train_dataset.o_state_dim + self.train_dataset.ft_state_dim
            else:
    ...
    def train(self, n_epochs, model_data_dir, no_wandb=True):
        train_dataloader = torch.utils.data.DataLoader(self.train_dataset, batch_size=128, shuffle=True)
        test_dataloader = torch.utils.data.DataLoader(self.test_dataset, batch_size=128, shuffle=False)
    ...
    def get_obs_and_state_next_from_batch(self, batch):
        ...
        obs = get_obs_vec_from_obs_dict(batch["obs"], use_ftpos=self.conf.algo.use_ftpos, device=self.device)
