...
from imitation_learning.utils.envs import full_env_registry
...
class PointMassEnv(VecEnv):
    def __init__(
        ...
        **kwargs,
        ...
        self._ep_rewards = []
        obs_space = spaces.Box(low=-1.5, high=1.5, shape=(2,), seed=seed)
    ...
    def step(self, action):
        ...
        all_is_done = torch.tensor([is_done], dtype=torch.bool)
        dist_to_goal = torch.linalg.norm(
            self._goal - self.cur_pos, dim=-1, keepdims=True
        )
        all_info = {"dist_to_goal": dist_to_goal}
        ...
        if is_done:
            ...
            all_info["episode"] = {
                "r": torch.stack(self._ep_rewards).sum(0),
                "max_action_magnitude": action_magnitudes.max().unsqueeze(0),
                "avg_action_magnitude": action_magnitudes.mean().unsqueeze(0),
            }
            all_info[FINAL_OBS_KEY] = final_obs
        else:
            all_info["episode"] = {
                "r": torch.stack(self._ep_rewards).sum(0),
                "max_action_magnitude": torch.tensor(0),
                "avg_action_magnitude": torch.tensor(0),
            }
            all_info[FINAL_OBS_KEY] = torch.zeros(self._get_obs().shape[0])

        return (obs, reward, all_is_done, all_info)
    ...
    def get_images(self, mode=None, img_dim=64, **kwargs) -> np.ndarray:
        ...
        img = write_to_img(img, agent_pos, entity_size, agent_color)
        img = write_to_img(img, goal_pos, entity_size, goal_color)
    ...
    def _get_dist_idx(self):
        ...
        if self._is_eval:
            ...
            if self._params.start_idx == -1:
                return torch.randint(0, self._params.num_train_regions, (1,))
            else:
                return self._params.start_idx

    def _sample_start(self, offset_start):
        idx = self._get_dist_idx()
        samples = self._start_distributions.sample()
        ang = samples[idx]
    ...
    def reset(self):
        self.cur_pos = self._sample_start(self._goal).squeeze(0)
        self._ep_step = 0
        self._ep_rewards = []
        self._prev_pos = self.cur_pos.detach().clone()
        self._store_actions = []
