...
from imitation_learning.utils.envs.pointmass import PointMassEnv, PointMassParams
...
class PointMassObstacleEnv(PointMassEnv):
    ...
    def _add_to_info(self, all_info):
        ...
        all_info["at_goal"] = dists.item() < self._params.goal_thresh
        return all_info

    def forward(self, cur_pos, action):
        action = action.to(self._device)
        action = torch.clamp(action, -1.0, 1.0)
        new_pos = cur_pos + (action * self._params.dt).unsqueeze(0)
