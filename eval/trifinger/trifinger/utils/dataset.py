...
import eaif_models.transforms as eaif_t
...
class BCFinetuneDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if self.obj_state_type in MODEL_NAMES:
            _, self.preproc_transform, pretrained_rep_dim = get_model_and_transform(
        ...
        self.rand_augment_transforms = eaif_t.transform_augment(
            # Resize/crop
            resize_size=256,
            output_size=224,
            # Jitter
