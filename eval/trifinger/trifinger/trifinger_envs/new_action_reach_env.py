...
class NewActionReachEnv(gym.Env):
    ...
    def reset(self, init_pose_dict=None, init_robot_position=None):
        ...
        if self.randomize_starts:
            ...
            observation = self.platform.simfinger.reset_finger_positions_and_velocities(
                action
            )
        ...
        #     ),
        #     "high": np.array([math.radians(70), 0, math.radians(-2)] * 3),
        # }
        # target_joint_config = np.asarray(
        #     sample.feasible_random_joint_positions_for_reaching(
        #         self.platform.simfinger, action_bounds
        #     )
        # )
