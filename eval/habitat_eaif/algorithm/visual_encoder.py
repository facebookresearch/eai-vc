...
class VisualEncoder(nn.Module):
    def __init__(
        ...
        if "resnet" in backbone_config.metadata.model:
            ...
            assert (
                global_pool and use_cls is False
            ), "Both global_pool and use_cls config param cant be 'True'"
            backbone_config.defrost()
            backbone_config.model.img_size = image_size
            backbone_config.model.global_pool = global_pool
            backbone_config.model.use_cls = use_cls
            backbone_config.freeze()
            self.backbone, embed_dim, _, _ = hydra.utils.call(backbone_config)

            # TODO: move this outside habitat codebase
            if backbone_config.model.global_pool or backbone_config.model.use_cls:
                self.compression = nn.Identity()
                self.output_size = embed_dim
            else:
                assert backbone_config.model.mask_ratio == 0.0
