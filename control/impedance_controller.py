...
class ImpedanceController:
    ...
    def get_command_torque(self, x_des, dx_des, q_cur, dq_cur, f_des=None):
        ...
        x_cur = np.array(self.kinematics.forward_kinematics(q_cur)).reshape((self.Nq,1))
    ...
    def is_avg_dx_converged(self, q_cur, dq_cur, epsilon=0.001):
        ...
        J_lin = self.kinematics.get_hand_lin_jacobian(q_cur) # Linear Jacobian for hand [9, Nq]
        ...
        for f_id in range(self.Nf):
            ...
            self._dx_mag_avg[f_id] = 0.5 * np.linalg.norm(dx_cur_i) + 0.5 * self._dx_mag_avg[f_id]
            all_fingers_converged = all_fingers_converged and self._dx_mag_avg[f_id] < epsilon
