...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(
        ...
        self.decoder_pos_embed = nn.Parameter(
            torch.zeros(decoder_max_offset + 1, num_patches + 1, decoder_embed_dim),
            requires_grad=False,
        )  # fixed sin-cos embedding
    ...
    def initialize_weights(self):
        ...
        decoder_pos_embed = get_3d_sincos_pos_embed(
            self.decoder_pos_embed.shape[-1], grid_size, time_size, cls_token=True
        )
    ...
    def forward_decoder(self, x1, x2, offsets, ids_restore1, ids_restore2):
        ...
        mask_tokens1 = self.mask_token.repeat(
            x1.shape[0], ids_restore1.shape[1] + 1 - x1.shape[1], 1
        )
        x1_ = torch.cat([x1[:, 1:, :], mask_tokens1], dim=1)  # no cls token
        x1_ = torch.gather(
            x1_, dim=1, index=ids_restore1.unsqueeze(-1).repeat(1, 1, x1.shape[2])
        )  # unshuffle
        x1 = torch.cat([x1[:, :1, :], x1_], dim=1)  # append cls token

        mask_tokens2 = self.mask_token.repeat(
            x2.shape[0], ids_restore2.shape[1] + 1 - x2.shape[1], 1
        )
        x2_ = torch.cat([x2[:, 1:, :], mask_tokens2], dim=1)  # no cls token
        x2_ = torch.gather(
            x2_, dim=1, index=ids_restore2.unsqueeze(-1).repeat(1, 1, x2.shape[2])
        )  # unshuffle
    ...
    def forward(
        ...
        (
            ...
            mask2,
            ids_restore2,
        ) = self.forward_encoder(extra_imgs1, extra_imgs2, mask_ratio1, mask_ratio2)

        pred1, pred2 = self.forward_decoder(
            latent1, latent2, offsets, ids_restore1, ids_restore2
        )  # [N, L, p*p*3]
...
mae_vit_small_patch16_tiny_decoder = (
    mae_vit_small_patch16_dec128d4b  # decoder: 128 dim, 4 blocks
)
mae_vit_small_patch16_large_decoder = (
    mae_vit_small_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
)
...
if __name__ == "__main__":
    ...
    imgs1 = torch.rand(N, 3, H, W, device=device)
    imgs2 = torch.rand(N, 3, H, W, device=device)
