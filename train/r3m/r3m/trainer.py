    ...
    def update(self, model, batch, step, eval=False):
        ...
        if model.module.langweight > 0:
            ...
            for _ in range(num_neg):
                ...
                sim_negs1.append(
                    model.module.get_reward(e0[negvidid], eg[negvidid], b_lang)[0]
                )
                negvidid = torch.randperm(e0.size()[0])
                sim_negs2.append(
                    model.module.get_reward(e0[negvidid], es1[negvidid], b_lang)[0]
                )
                negvidid = torch.randperm(e0.size()[0])
                sim_negs3.append(
                    model.module.get_reward(e0[negvidid], es2[negvidid], b_lang)[0]
                )
            ...
            rewloss1 = -torch.log(
                ...
                + (
                    torch.exp(sim_pos1)
                    / (epsilon + torch.exp(sim_pos1) + sim_negs_exp1.sum(-1))
                )
            )
            rewloss2 = -torch.log(
                epsilon
                + (
                    torch.exp(sim_pos2)
                    / (epsilon + torch.exp(sim_pos2) + sim_negs_exp2.sum(-1))
                )
            )
            rewloss3 = -torch.log(
                epsilon
                + (
                    torch.exp(sim_pos3)
                    / (epsilon + torch.exp(sim_pos3) + sim_negs_exp3.sum(-1))
                )
            )
