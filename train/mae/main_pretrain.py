...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--data_path', nargs='+', default=['/datasets01/imagenet_full_size/061417/'], type=str,
                        help='dataset path')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            misc.save_model(
                ...
                loss_scaler=loss_scaler, epoch=epoch, model_name=('checkpoint.pth'))
            if (epoch % 20 == 0 or epoch + 1 == args.epochs):
                misc.save_model(
                    args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                    loss_scaler=loss_scaler, epoch=epoch, model_name=('checkpoint-%s.pth' % str(epoch)))
