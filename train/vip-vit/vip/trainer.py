...
class Trainer:
    ...
    def update(self, model, batch, step, eval=False):
        ...
        V_loss = (1 - model.module.gamma) * -V_0.mean() + torch.log(
            epsilon + torch.mean(torch.exp(-(r + model.module.gamma * V_s_next - V_s)))
        )
        ...
        if model.module.num_negatives > 0:
            ...
            V_loss = V_loss + torch.log(
                epsilon
                + torch.mean(
                    torch.exp(-(r_neg + model.module.gamma * V_s_next_neg - V_s_neg))
                )
            )
