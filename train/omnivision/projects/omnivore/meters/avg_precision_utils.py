# Copied as is from AP computation in VISSL
# https://github.com/facebookresearch/vissl/blob/main/vissl/utils/svm_utils/evaluate.py
...
def get_precision_recall(targets, scores, weights=None):
    ...
    if numinst > 0:
        ...
        # If there is no GT, AP is not defined. This can happen at intermediate points
        # when some metric update() has been called but it hasn't seen the
        # GT for some classes.
        R = np.empty_like(tp)
        R[:] = np.nan
        ap = np.array([np.nan])
