...
from omnivore.losses import LossWithUpdatedOutput
...
class CrossEntropyMultipleOutputSingleTargetLoss(nn.Module):
    ...
    def forward(
        ...
        from omnivision.utils.debug import replace_tensor_with_metadata

        logging.info(
            f"{replace_tensor_with_metadata(output)}. {replace_tensor_with_metadata(target)}"
        )
        loss = 0
