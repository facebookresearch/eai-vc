...
class VideoPathDatasetFromClips(Dataset):
    def __init__(
        ...
        logging.info(
            f"Created dataset from {self.path_file_list} of length: {self.num_samples:,} ({self.num_frames:,} frames)"
        )

    def _load_data(self):
        logging.info(f"Loading {self.path_file_list} with shared memory")
        self.paths, self.clips = self.sm_loader.load(
            self.path_file_list,
    ...
    def sample_frames(self, idx: int) -> List[str]:
        video_start, video_stop = self.clips[idx]
        all_frames = list(range(video_start, video_stop + 1, self.every_k))
        all_starts = list(range(len(all_frames) - self.frames_per_video + 1))
        clip_start = np.random.choice(all_starts)
        frames = all_frames[clip_start : clip_start + self.frames_per_video]
        if self.randomly_reverse and np.random.rand() > 0.5:
            frames = list(reversed(frames))
        return [self.paths[f] for f in frames]
    ...
    def try_load_object(self, idx):
        is_success = True
        try:
            paths = self.sample_frames(idx)
            data = self.load_object(paths)
        except Exception:
            logging.warning(f"Couldn't load: {paths}.")
            logging.debug("Exception: ", exc_info=True)
            is_success = False
            data = self.default_generator()
        return data, is_success
