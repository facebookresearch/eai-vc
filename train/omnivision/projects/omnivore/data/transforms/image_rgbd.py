...
class DepthToInverseDepth(torch.nn.Module):
    def __init__(
        self,
        scale_depth_before_inv: float = None,
        min_depth: float = 0.1,
        scale: float = 1,
        ...
        self.scale_depth_before_inv = scale_depth_before_inv
    ...
    def __call__(self, image: torch.Tensor):
        ...
        if self.scale_depth_before_inv:
            depth_img *= self.scale_depth_before_inv

        disparity = self.scale * 1 / depth_img.clamp(min=min_depth, max=max_depth)

        if C == 4:
            disparity = torch.cat([image[:3, ...], depth_img])
        return disparity
...
class DepthNorm(torch.nn.Module):
    ...
    def __init__(
        ...
        compute_max_per_sample: bool = False,
    ):
        """
        Args:
        ...
        self.compute_max_per_sample = compute_max_per_sample
        if self.compute_max_per_sample:
    ...
    def __call__(self, image: torch.Tensor):
        ...
        if self.compute_max_per_sample:
            max_depth = depth_img.max().item()
        else:
            max_depth = self.max_depth

        # clamp to max
        if self.clamp_max_before_scale:
            depth_img = depth_img.clamp(max=max_depth)
        # divide by max_depth
        if self.scale_depth:
            depth_img /= max_depth

        if C == 4:
            depth_img = torch.cat([image[:3, ...], depth_img], dim=0)
        return depth_img

    def extra_repr(self):
        return f"max_depth={self.max_depth}, clamp_max_before_scale={self.clamp_max_before_scale}, min_depth={self.min_depth}, scale_depth={self.scale_depth}, compute_max_per_sample={self.compute_max_per_sample}"
