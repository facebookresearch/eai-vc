...
class CutMixUp(Callable):
    ...
    def __call__(self, batch):
        ...
        """
        assert isinstance(batch, VisionSample), "Only one supported yet"
...
class Mixup:
    ...
    def __call__(self, sample):
        ...
        if len(orig_shape) == 5:
            ...
            x = x.view(orig_shape)
        # # rgirdhar: Debugging code to visualize the output
        # for i in range(len(x)):
        #     vid = x[i].permute(1, 2, 3, 0)
        #     vid = vid - vid.min()
        #     vid /= vid.max()
        #     vid *= 255.0
        #     import torchvision
        #     torchvision.io.write_video(f"/tmp/rgirdhar/vid_{i}.mp4",
        #                                vid.numpy().astype('uint8'), fps=12)
        # import pdb; pdb.set_trace()
        # mannatsingh: similar debugging code for images
        # for i in range(len(x)):
        #     im = x[i]
        #     im = im - im.min()
        #     im /= im.max()
        ...
        #     save_image(im, f"/tmp/mannatsingh/{i}.jpg")
