...
def open_loop(cfg: dict):
	...
	for i in range(cfg.eval_episodes):
		...
		imageio.mimsave(openloop_dir / f'h{cfg.horizon}_{i}.gif', torch.cat((pred_frames, gt_frames), dim=2))
		metrics.append(np.array([i, cfg.modality, cfg.exp_name, cfg.horizon, cfg.seed, ep_reward]))
		pd.DataFrame(np.array(metrics)).to_csv(f'{openloop_dir}/metrics.csv', header=['episode', 'modality', 'exp_name', 'horizon', 'seed', 'episode_reward'], index=None)
